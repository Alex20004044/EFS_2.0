...
namespace MSFD
{
    ...
    {
        SortedDictionary<int, List<Func<T, T>>> modsDictionary = new SortedDictionary<int, List<Func<T, T>>>();
        ...
        public IDisposable AddMod(Func<T, T> mod, int priority = 0)
        {
            priority = -priority;
            List<Func<T, T>> mods;
            if (!this.modsDictionary.TryGetValue(priority, out mods))
            {
                mods = new List<Func<T, T>>();
                this.modsDictionary.Add(priority, mods);
            }
            mods.Add(mod);
        ...
        {
            foreach (var x in modsDictionary)
            {
