...
{
	[SerializeField]
	bool isRecieveMessage = false;
	[SerializeField]
	TesterForce testerForce;
	[SerializeField]
	float speed = 0.1f;
	...
	protected virtual void OnMessageReceived(string receivedMessage)
	{
		...
		if (isRecieveMessage)
		{
			MoveSensor moveSensor = JsonUtility.FromJson<MoveSensor>(receivedMessage);
			Debug.Log(moveSensor.displacement);
			testerForce.transform.position = testerForce.transform.position + moveSensor.displacement * speed * waitingMessagesFrequency;
		}
...
struct MoveSensor
{
	...
	public MoveSensor(Vector3 displacement)
	{
